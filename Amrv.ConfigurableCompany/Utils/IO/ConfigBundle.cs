...
namespace Amrv.ConfigurableCompany.Utils.IO
{
    public class ConfigBundle
    {
        ...
        private readonly Dictionary<string, ConfigEntry> _entries = [];
        ...
        public ConfigEntry AddEntry(string name, string value, Dictionary<string, string> meta = null)
        {
            if (TryGetEntry(name, out ConfigEntry entry))
            {
                entry.Value = value;
            ...
            {
                entry = new ConfigEntry() { Value = value };
                _entries[name] = entry;
        ...
        public bool TryGetEntry(string name, out ConfigEntry entry) => _entries.TryGetValue(name, out entry);

        public IReadOnlyDictionary<string, ConfigEntry> Entries() => _entries;


        public void Read(in string data)
        {
            _meta.Clear();
            _entries.Clear();
            ReadEntries(data);
        ...
        public void Write(out string data)
        {
